// utils/smartSchema.js
// Atualizações seguras e idempotentes para as novas áreas inteligentes do painel.
// Não altera .env e não apaga dados existentes.

let alreadyEnsured = false;
let ensuringPromise = null;

async function ensureSmartSchema(pool) {
  ...
  ensuringPromise = (async () => {
    ...
    await pool.query(`
      INSERT INTO smart_finance_configs (id)
      VALUES (1)
      ON CONFLICT (id) DO NOTHING;

      INSERT INTO auto_promotion_settings (id)
      VALUES (1)
      ON CONFLICT (id) DO NOTHING;
    `);
...
module.exports = { ensureSmartSchema };
